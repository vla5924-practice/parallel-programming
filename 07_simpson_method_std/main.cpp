...
/*TEST(StdThread_SimpsonMethodTest, same_result_as_sequential) {
...
TEST(StdThread_SimpsonMethodTest, can_integrate_2d_function) {
    std::vector<double> seg_begin = {0};
    std::vector<double> seg_end = {2};
    double square = SimpsonMethod::parallel(parabola, seg_begin, seg_end, 100, std::thread::hardware_concurrency());
    ASSERT_NEAR(16.0 / 3.0, square, 1e-6);
}

TEST(StdThread_SimpsonMethodTest, can_integrate_3d_function) {
    std::vector<double> seg_begin = {0, 0};
    std::vector<double> seg_end = {1, 1};
    double volume = SimpsonMethod::parallel(body, seg_begin, seg_end, 100, std::thread::hardware_concurrency());
...
TEST(StdThread_SimpsonMethodTest, can_integrate_super_function) {
    std::vector<double> seg_begin = {-2, 1, 0};
    std::vector<double> seg_end = {1, 3, 2};
    double integral = SimpsonMethod::parallel(super, seg_begin, seg_end, 100, std::thread::hardware_concurrency());
    ASSERT_NEAR(13.0007625, integral, 1e-6);
}

TEST(StdThread_SimpsonMethodTest, cannot_accept_empty_segment_vectors) {
    ASSERT_ANY_THROW(SimpsonMethod::parallel(generic, {}, {}, 100));
    ASSERT_ANY_THROW(SimpsonMethod::parallel(generic, {0}, {}, 100));
    ASSERT_ANY_THROW(SimpsonMethod::parallel(generic, {}, {0}, 100));
}

TEST(StdThread_SimpsonMethodTest, cannot_accept_invalid_segment_vectors) {
...
TEST(StdThread_SimpsonMethodTest, cannot_accept_invalid_steps_count) {
